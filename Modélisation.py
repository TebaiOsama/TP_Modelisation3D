...
def CameraCalibration():
    # Defines the number of iterations and the error
    criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    # prepare object points, like (0,0,0), (1,0,0), (2,0,0) ....,(6,5,0)
    ############ to adapt ##########################
    objp = np.zeros((4*6, 3), np.float32)
    objp[:, :2] = np.mgrid[0:4, 0:6].T.reshape(-1, 2)
    # Scale of the checkboard
    objp[:, :2] *= 40
    #################################################
    # Creation of the arrays
    objpoints = []  # Points of the change of colour
    imgpoints = []  # Points of the angles
    ############ to adapt ##########################
    ...
    for fname in images:
        ...
        # Image in scale of grey
        ############ to adapt ##########################
        ret, corners = cv.findChessboardCorners(gray, (4, 6), None)
        #################################################
        print(ret)
        # If we find the edges of the chessboard
        if ret == True:
            objpoints.append(objp)
            corners2 = cv.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)
            imgpoints.append(corners)
            # To print the edges that we've found
            ############ to adapt ##########################
    ...
    # Finds the intrinsic and extrinsic parameters
    ...
    # Calculation of the non distortion and rectification map
    mapx, mapy = cv.initUndistortRectifyMap(mtx, dist, None, newcameramtx, (w, h), 5)
    dst = cv.remap(img, mapx, mapy, cv.INTER_LINEAR)
    # Prints the last image and waits for input
    x, y, w, h = roi
    dst = dst[y:y + h, x:x + w]
    cv.namedWindow('img', 0)
    cv.imshow('img', dst)
    cv.waitKey(0)
    ############ to adapt ##########################
    cv.imwrite('Images/3d/calibresultM.png', dst)
    #################################################
...
def DepthMapfromStereoImages():
    ############ to adapt ##########################
    imgL = cv.pyrDown(cv.imread('Images/3d/aloeL.jpg'))
    imgR = cv.pyrDown(cv.imread('Images/3d/aloeR.jpg'))
    #################################################
    # Window creation
    window_size = 3
    min_disp = 16
    ...
    speckleRange = 32)
    # Calculating the disparity between 2 near images
...
def StereoCalibrate(Cameramtx):
    ############ to adapt ##########################
    img1 = cv.pyrDown(cv.imread('Images/3d/leftT2.jpg', 0))
    img2 = cv.pyrDown(cv.imread('Images/3d/rightT2.jpg', 0))
    #################################################
    # opencv 4.5
    sift = cv.SIFT_create()
    # opencv 3.4
    ...
    print('E\n',E)
    # Returns the relative position and the movement between 2 images
...
def EpipolarGeometry(pts1, pts2, F, maskF, FT, maskE):
    ...
    # Correcting calibre errors
    retval, H1, H2 = cv.stereoRectifyUncalibrated(pts1, pts2, F, (c,r))
    print(H1)
    print(H2)
    # Changes perspective to get matching images
